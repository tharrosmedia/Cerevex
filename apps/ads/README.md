...
Modularity retrofit + M5.1 Brief 1.6 + M5.2 Phase A/B: product capabilities live in `settings_json.capabilities`. `m51.*`, `m52.callrail_connect`, `m52.bundled_call_tracking`, `m52.crm_join`, and `apply.create_entity` are live flags (default hidden). `modules.leads` is IA only — the Leads / brainstorm surface also needs `m51.brainstorm` visible. Connector interfaces (`AdPlatformConnector`, `AnalyticsConnector`, `CallTrackingConnector`) live in `@tharros/ads-shared/connectors`. CallRail Connect and bundled Twilio-class tracking swap behind the same interface. Sync pull, live apply, and OAuth exchange go through `getAdPlatformConnector`. Details: [`docs/modularity-retrofit.md`](../../docs/modularity-retrofit.md) and [`docs/m51-brief-1.6.md`](../../docs/m51-brief-1.6.md).
