...
Modularity retrofit: product capabilities (`cockpit`, `apply`, `connect.meta`, `connect.google`, `audits`, plus `m51.*` dark placeholders) live in the same `settings_json.capabilities` object. Unfinished `m51.*` stay out of operator Settings and cannot be set to `on`. `modules.leads` is IA only — the Leads / brainstorm placeholder also needs `m51.brainstorm` visible. Connector interfaces (`AdPlatformConnector`, `AnalyticsConnector`, `CallTrackingConnector`) live in `@tharros/ads-shared/connectors`. Sync pull, live apply, and OAuth exchange go through `getAdPlatformConnector` — Meta/Google branches stay inside those implementations. Details: [`docs/modularity-retrofit.md`](../../docs/modularity-retrofit.md).
