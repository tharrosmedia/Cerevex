...
Modularity retrofit + M5.1 Brief 1.6: product capabilities live in `settings_json.capabilities`. `m51.*` and `apply.create_entity` are live flags (default hidden) — operators can turn them on. `modules.leads` is IA only — the Leads / brainstorm surface also needs `m51.brainstorm` visible. Connector interfaces (`AdPlatformConnector`, `AnalyticsConnector`, `CallTrackingConnector`) live in `@tharros/ads-shared/connectors`. Sync pull, live apply, and OAuth exchange go through `getAdPlatformConnector`. GA4 + first-party pixel use `AnalyticsConnector`. Details: [`docs/modularity-retrofit.md`](../../docs/modularity-retrofit.md) and [`docs/m51-brief-1.6.md`](../../docs/m51-brief-1.6.md).
