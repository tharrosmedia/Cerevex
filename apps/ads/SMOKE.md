# Ads module smoke: shared Neon gate + M3 mock path
...
**OS process** (`apps/ads/.env` — not Brain `APP_PASSWORD`):
...
- Always set `OS_INNGEST_APP_ID=cerevex-ads` when OS can see Brain env.
...
```bash
# already part of npm run ads:dev — Inngest Dev :43183 → worker :43182/api/inngest
...
- `npm run ads:db:seed` on production Brain Neon
...
`npm run ads:dev` registers `os-audit-requested` on app id `cerevex-ads` / `OS_INNGEST_APP_ID`.
...
Operator shell: `http://127.0.0.1:43181` after `npm run ads:dev` (API `:43180`). Sign in as the seeded owner. Leave Meta/Google app IDs empty.
