...
import { z } from "zod";
import { EVENTS, canMutate } from "@tharros/shared";
...
} from "@tharros/shared/audit";
import { getDb } from "@tharros/shared/db";
import { sendApplyRequested, sendAuditRequested } from "@tharros/shared/inngest";
import { applyJobs, workspaces } from "@tharros/shared/schema";
  ...
  app.post("/clients/:id/audits", requireAuth, async (c) => {
    ...
    try {
      ...
      throw new HTTPException(503, {
        message:
          "Inngest is not reachable. Use { \"inline\": true } for mock-mode smoke, or start the local Dev Server (npm run os:dev).",
      });
    }
  });
