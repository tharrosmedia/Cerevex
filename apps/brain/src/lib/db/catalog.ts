...
export async function upsertCatalogResource(storeId: string, r: any) {
  ...
  const result = await sql`
    INSERT INTO catalog_resources (store_id, shopify_id, resource_type, handle, title, seo_title, seo_description, body_html, metafields, product_count, published, shopify_updated_at, synced_at, connector_type, external_id)
    VALUES (${storeId}, ${r.shopifyId}, ${r.resourceType}, ${r.handle || null}, ${r.title || null}, ${r.seoTitle || null}, ${r.seoDescription || null}, ${r.bodyHtml || null}, ${safeMf}, ${r.productCount ?? null}, ${r.published ?? null}, ${r.shopifyUpdatedAt || null}, now(), ${r.connectorType || 'shopify'}, ${r.externalId || r.shopifyId || null})
...
export async function getCatalogResource(id: string) {
  const sql = neon(process.env.DATABASE_URL!);
  const res = await sql`SELECT id, store_id as "storeId", shopify_id as "shopifyId", resource_type as "resourceType", handle, title, seo_title as "seoTitle", seo_description as "seoDescription", body_html as "bodyHtml", metafields, product_count as "productCount", published, shopify_updated_at as "shopifyUpdatedAt", synced_at as "syncedAt", connector_type as "connectorType", external_id as "externalId" FROM catalog_resources WHERE id = ${id} LIMIT 1`;
  return res[0] || null;
}
