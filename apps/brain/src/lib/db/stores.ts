...
export async function listStores() {
  ...
  return sql`SELECT id, name, shopify_domain, platform, connector_type, config, created_at FROM stores ORDER BY created_at DESC`;
}
...
export async function updateStore(id: string, { name, shopify_domain, shopify_access_token, platform = 'shopify', connector_type, config, clearToken }: { name: string; shopify_domain: string; shopify_access_token: string; platform?: string; connector_type?: string; config?: any; clearToken?: boolean }) {
  const sql = neon(process.env.DATABASE_URL!);
  const safeConfig = config !== undefined ? toSafeJsonb(config, 'store.config') : null;
  const connectorType = connector_type || platform || 'shopify';
  if (clearToken) {
    const result = await sql`UPDATE stores SET name = ${name}, shopify_domain = ${shopify_domain}, shopify_access_token = null, platform = ${platform}, connector_type = ${connectorType}, config = ${safeConfig}, updated_at = now() WHERE id = ${id} RETURNING id, name, shopify_domain, platform, connector_type, config, created_at`;
    return result[0];
  }
  if (shopify_access_token && shopify_access_token.length > 0) {
    ...
    const result = await sql`UPDATE stores SET name = ${name}, shopify_domain = ${shopify_domain}, platform = ${platform}, connector_type = ${connectorType}, config = ${safeConfig}, updated_at = now() WHERE id = ${id} RETURNING id, name, shopify_domain, platform, connector_type, config, created_at`;
    return result[0];
  }
}
